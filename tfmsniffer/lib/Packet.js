...
class ByteArray {
	...
	/**
	 * Reads a stream of bytes from the byte stream.
	 * @param {Number} size - The number of bytes to read
	 * @returns {Buffer}
	 */
	readBytes(size) {
		const value = this.buffer.subarray(this.readPosition, this.readPosition + size);
	 ...
	 * @returns {Number} - The signed 8-bit integer that was readed from the byte stream.
	 */
	read8() {
	...
	/**
	 * Reads a string buffer from the byte stream.
	 * @returns {Buffer} - String buffer
	 */
	readString() {
...
/* Represents a Transformice packet */
class Packet extends ByteArray {
	readCode() {
